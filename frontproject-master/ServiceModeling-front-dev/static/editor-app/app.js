  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId) {
                ...
                $http({method: 'GET', url: KISBPM.URL.getModel(modelId)}).
                    success(function (res, status, headers, config) {
                        $rootScope.editor = new ORYX.Editor(res.data);
                        $rootScope.modelData = angular.fromJson(res.data);
                        $rootScope.editorFactory.resolve();
                        $http({method: 'GET', url: KISBPM.URL.getFolderGoals(res.data.folderId)}).
                        success(function (res, status, headers, config) {
                            $rootScope.goals = angular.fromJson(res.data);
                        }).
                        error(function (data, status, headers, config) {
                            console.log('Error loading model goals with id ' + modelId + ' ' + data);
                        });
