    ...
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            /*
             StencilSet items
             */
            $http({method: 'GET', url: KISBPM.URL.getStencilSet($rootScope.modelData.type)}).success(function (data, status, headers, config) {

            	var quickMenuDefinition = null;
            	if($rootScope.modelData.type==='BPMN')
                quickMenuDefinition = ['UserTask', 'EndNoneEvent', 'ExclusiveGateway',
                  'CatchTimerEvent', 'ThrowNoneEvent', 'TextAnnotation', 'SequenceFlow', 'Association'];
            	else
            	  quickMenuDefinition = [ 'ValueActivity', 'FunctionalObjective', 'OrFlow', 'AndFlow',
                  'PromoteFlow', 'BlockFlow', 'MutexFlow', 'RelyFlow', 'SequenceFlow', 'Association', 'PromoteFlow2'];
            ...
            if ($scope.dragCanContain) {
            	...
            	if ($scope.quickMenu)
            	{
            		...
            		if (shapes && shapes.length == 1)
            		{
	        			...
	        			if (containedStencil.idWithoutNs() !== 'OrFlow' && containedStencil.idWithoutNs() !== 'AndFlow' && containedStencil.idWithoutNs() !== 'PromoteFlow' && containedStencil.idWithoutNs() !== 'BlockFlow'
                    && containedStencil.idWithoutNs() !== 'MutexFlow' && containedStencil.idWithoutNs() !== 'RelyFlow' && containedStencil.idWithoutNs() !== 'SequenceFlow'
                    && containedStencil.idWithoutNs() !== 'Association' && containedStencil.idWithoutNs() !== 'MessageFlow' && containedStencil.idWithoutNs() !== 'DataAssociation')
	        			{
