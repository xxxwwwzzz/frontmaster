...
<!--                <b-form-radio v-model="selectedFormat" name="orad" value="XML">XML file</b-form-radio>-->
        ...
        <!--重命名-->
        <b-navbar-nav>
          <span class="navbar-brand rename" id="project-name" ref="projectName" v-b-tooltip.hover
                title="Click to rename this project">{{diagram.diagramName}}</span>
        </b-navbar-nav>
        <!--// TODO 搜索框align-right-->
    ...
    <b-row style="position:absolute;top: 50px;bottom: 20px;left: 0px;right: 0px;">
      ...
      <b-col style="height: 100%">
        ...
        <div
          ...
          <button title="折叠侧边栏" class="tool-bar-btn" @click="showSideBar=!showSideBar;moveCanvasCenter();"><i
            class="fas fa-bars"></i></button>
        </div>
        <div id="dock">
          <div class="dock-header"></div>
          <div class="dock-buttons">
            <div id="dock-btn-navigator" title="Navigator" class="tool-bar-btn" @click="setDock(1)"><i
              class="fa fa-crosshairs"></i></div>
            <div id="dock-btn-attribute" title="Data Attributes" class="tool-bar-btn" @click="setDock(2)"><i
              class="fa fa-table"></i></div>
...
export default {
  ...
  data () {
    return {
      ...
      selectGrids: [
        {text: '无', value: null},
        {text: '小', value: 'small-size'},
        {text: '正常', value: 'normal-size'},
        {text: '大', value: 'big-size'},
        {text: '更大', value: 'bigger-size'}
      ],
      flag_showgrid: true,
      search_content: '',
      search_type: null,
  ...
  mounted () {
    ...
    const GradientYellow = objGo(go.Brush, 'Linear', {0: 'LightGoldenRodYellow', 1: '#FFFF66'})
    const GradientLightGreen = objGo(go.Brush, 'Linear', {0: '#E0FEE0', 1: 'PaleGreen'})
    const GradientLightGray = objGo(go.Brush, 'Linear', {0: 'White', 1: '#DADADA'})

    const ActivityNodeFill = objGo(go.Brush, 'Linear', {0: 'OldLace', 1: 'PapayaWhip'})
    ...
    const activityNodeMenu =
                objGo('ContextMenu',
                  objGo('ContextMenuButton', objGo(go.TextBlock, 'Add Email Event', {margin: 3}), { click: function (e, obj) { addActivityNodeBoundaryEvent(2, 5) } }),
                  objGo('ContextMenuButton', objGo(go.TextBlock, 'Add Timer Event', {margin: 3}), { click: function (e, obj) { addActivityNodeBoundaryEvent(3, 5) } }),
                  objGo('ContextMenuButton', objGo(go.TextBlock, 'Add Escalation Event', {margin: 3}), { click: function (e, obj) { addActivityNodeBoundaryEvent(4, 5) } }),
                  objGo('ContextMenuButton', objGo(go.TextBlock, 'Add Error Event', {margin: 3}), { click: function (e, obj) { addActivityNodeBoundaryEvent(7, 5) } }),
                  objGo('ContextMenuButton', objGo(go.TextBlock, 'Add Signal Event', {margin: 3}), { click: function (e, obj) { addActivityNodeBoundaryEvent(10, 5) } }),
                  objGo('ContextMenuButton', objGo(go.TextBlock, 'Add N-I Escalation Event', {margin: 3}), { click: function (e, obj) { addActivityNodeBoundaryEvent(4, 6) } }),
                  objGo('ContextMenuButton', objGo(go.TextBlock, 'Rename', {margin: 3}), { click: function (e, obj) { rename(obj) } }))
    ...
    function makeMarkerPanel (sub, scale) {
      return objGo(go.Panel, 'Horizontal',
        ...
        objGo(go.Shape, 'BpmnActivityLoop', { width: 12 / scale, height: 12 / scale, margin: 2, visible: false, strokeWidth: ActivityMarkerStrokeWidth }, new go.Binding('visible', 'isLoop')),
        objGo(go.Shape, 'BpmnActivityParallel', { width: 12 / scale, height: 12 / scale, margin: 2, visible: false, strokeWidth: ActivityMarkerStrokeWidth }, new go.Binding('visible', 'isParallel')),
        objGo(go.Shape, 'BpmnActivitySequential', { width: 12 / scale, height: 12 / scale, margin: 2, visible: false, strokeWidth: ActivityMarkerStrokeWidth }, new go.Binding('visible', 'isSequential')),
        objGo(go.Shape, 'BpmnActivityAdHoc', { width: 12 / scale, height: 12 / scale, margin: 2, visible: false, strokeWidth: ActivityMarkerStrokeWidth }, new go.Binding('visible', 'isAdHoc')),
        objGo(go.Shape, 'BpmnActivityCompensation', { width: 12 / scale, height: 12 / scale, margin: 2, visible: false, strokeWidth: ActivityMarkerStrokeWidth, fill: null }, new go.Binding('visible', 'isCompensation')),
        makeSubButton(sub)
    ...
    const privateProcessNodeTemplate =
                objGo(go.Node, 'Auto',
                  ...
                  objGo(go.Panel, 'Table', // table with 2 cells to hold header and lane
                    ...
                    new go.Binding('desiredSize', 'size', go.Size.parse).makeTwoWay(go.Size.stringify),
                    objGo(go.TextBlock,
                      { row: 0, column: 0, angle: 270, margin: 5, editable: true, textAlign: 'center' },
                      new go.Binding('text').makeTwoWay()),
                    objGo(go.RowColumnDefinition, {column: 1, separatorStrokeWidth: 1, separatorStroke: 'black'}),
                    objGo(go.Shape, 'Rectangle',
                      {
                        row: 0,
                        column: 1,
                ...
                objGo(go.Group, 'Spot', groupStyle(),
                  ...
                  objGo(go.Panel, 'Horizontal',
                    ...
                    objGo(go.TextBlock, // the lane label
                      ...
                      new go.Binding('text', 'text').makeTwoWay()),
                    objGo('SubGraphExpanderButton', {margin: 4, angle: -270}) // but this remains always visible!
                  ), // end Horizontal Panel
                  objGo(go.Placeholder,
                    {padding: 12, alignment: go.Spot.TopLeft, alignmentFocus: go.Spot.TopLeft}),
                  objGo(go.Panel, 'Horizontal', {alignment: go.Spot.TopLeft, alignmentFocus: go.Spot.TopLeft},
                ...
                objGo(go.Group, 'Auto', groupStyle(),
                  {
                    ...
                    layout: objGo(PoolLayout, {spacing: new go.Size(0, 0)}) // no space between lanes
                  },
                  new go.Binding('location', 'loc', go.Point.parse).makeTwoWay(go.Point.stringify),
                  objGo(go.Shape,
                    {fill: 'white'},
    ...
    const sequenceLinkTemplate =
                objGo(go.Link,
                  ...
                  objGo(go.Shape, {stroke: 'black', strokeWidth: 1}),
                  objGo(go.Shape, {toArrow: 'Triangle', scale: 1.2, fill: 'black', stroke: null}),
                  objGo(go.Shape, {fromArrow: '', scale: 1.5, stroke: 'black', fill: 'white'},
    ...
    const messageFlowLinkTemplate =
                objGo(PoolLink,
                  ...
                  new go.Binding('points').makeTwoWay(),
                  objGo(go.Shape, {stroke: 'black', strokeWidth: 1, strokeDashArray: [6, 2]}),
                  objGo(go.Shape, {toArrow: 'Triangle', scale: 1, fill: 'white', stroke: 'black'}),
                  objGo(go.Shape, {fromArrow: 'Circle', scale: 1, visible: true, stroke: 'black', fill: 'white'}),
                  objGo(go.TextBlock, {
    ...
    const dataAssociationLinkTemplate =
                objGo(go.Link,
                  ...
                  new go.Binding('points').makeTwoWay(),
                  objGo(go.Shape, {stroke: 'black', strokeWidth: 1, strokeDashArray: [1, 3]}),
                  objGo(go.Shape, {toArrow: 'OpenTriangle', scale: 1, fill: null, stroke: 'blue'})
                )
    ...
    const annotationAssociationLinkTemplate =
                objGo(go.Link,
                  ...
                  new go.Binding('points').makeTwoWay(),
                  objGo(go.Shape, {stroke: 'black', strokeWidth: 1, strokeDashArray: [1, 3]}),
                  objGo(go.Shape, {toArrow: 'OpenTriangle', scale: 1, stroke: 'black'})
                )
    ...
    const myPaletteDiv1 =
                objGo(go.Palette, 'myPaletteDiv1',
...
//         nodeDataArray: [
//           // -------------------------- Event Nodes
//           { key: 101, category: 'event', text: '启动', eventType: 1, eventDimension: 1, item: 'start' },
//           { key: 102, category: 'event', text: '消息启动', eventType: 2, eventDimension: 2, item: 'startMessage' }, // BpmnTaskMessage
//           { key: 103, category: 'event', text: '定时启动', eventType: 3, eventDimension: 3, item: 'startTimer' },
//           { key: 107, category: 'event', text: '消息结束', eventType: 2, eventDimension: 8, item: 'endMessage' }, // BpmnTaskMessage
//           { key: 104, category: 'event', text: '结束', eventType: 1, eventDimension: 8, item: 'End' },
//           { key: 108, category: 'event', text: '终止结束', eventType: 13, eventDimension: 8, item: 'endTerminate' },
//           // -------------------------- Task/Activity Nodes
//           { key: 131, category: 'activity', text: '任务', item: 'generic task', taskType: 0 },
//           { key: 132, category: 'activity', text: '用户任务', item: 'User task', taskType: 2 },
//           { key: 133, category: 'activity', text: '服务任务', item: 'service task', taskType: 6 },
//           // subprocess and start and end
//           { key: 134, category: 'subprocess', loc: '0 0', text: '子流程', isGroup: true, isSubProcess: true, taskType: 0, item: 'generic subprocess' },
//           { key: -802, category: 'event', loc: '0 0', group: 134, text: '启动', eventType: 1, eventDimension: 1, item: 'start' },
//           { key: -803, category: 'event', loc: '350 0', group: 134, text: '结束', eventType: 1, eventDimension: 8, item: 'End', name: 'end' },
//           // -------------------------- Gateway Nodes, Data, Pool and Annotation
//           { key: 201, category: 'gateway', text: '并行网关', gatewayType: 1 },
//           { key: 204, category: 'gateway', text: '排他网关', gatewayType: 4 },
//           { key: 301, category: 'dataobject', text: '数据对象' },
//           { key: 302, category: 'datastore', text: '数据存储' },
//           { key: 401, category: 'privateProcess', text: '折叠池' },
//           { key: '501', text: '池', isGroup: 'true', category: 'Pool' },
//           { key: 'Lane5', text: '泳道 1', isGroup: 'true', group: '501', color: 'lightyellow', category: 'Lane' },
//           { key: 'Lane6', text: '泳道 2', isGroup: 'true', group: '501', color: 'lightgreen', category: 'Lane' },
//           { key: 701, category: 'annotation', text: ' 注释 ' }
//         ] // end nodeDataArray
//       }) // end model
// 
...
//         copiesArrayObjects: true,
//         nodeDataArray: [
//           { key: 1, category: 'activity', taskType: 1, text: '接收任务', item: 'Receive Task' },
//           { key: 2, category: 'activity', taskType: 5, text: '发送任务', item: 'Send Task' },
//           { key: 3, category: 'activity', taskType: 7, text: '业务规则\n任务', item: 'Business Rule Task' },
//           // { key: 4, category: 'activity', taskType: 2, text: 'User Task', item: 'User Task', isCall: true },
//           { key: 101, text: 'Ad-hoc\n子流程', isGroup: true, isSubProcess: true, category: 'subprocess', isAdHoc: true, taskType: 0, loc: '0 0', item: 'adhc subprocess' },
//           { key: -812, group: 101, category: 'event', text: '启动', eventType: 1, eventDimension: 1, item: 'start', loc: '0 0' },
//           { key: -813, group: 101, category: 'event', text: '结束', eventType: 1, eventDimension: 8, item: 'end', name: 'end', loc: '350 0' },
//           { key: 102, text: '交易\n子流程', isGroup: true, isSubProcess: true, category: 'subprocess', isTransaction: true, taskType: 0, loc: '0 0', item: 'transactional subprocess' },
//           { key: -822, group: 102, category: 'event', text: '启动', eventType: 1, eventDimension: 1, item: 'start', loc: '0 0' },
//           { key: -823, group: 102, category: 'event', text: '结束', eventType: 1, eventDimension: 8, item: 'end', name: 'end', loc: '350 0' },
//           { key: 103, text: '循环\n子流程', isGroup: true, isLoop: true, isSubProcess: true, category: 'subprocess', taskType: 0, loc: '0 0', item: 'looping activity' },
//           { key: -831, group: 103, category: 'event', text: '启动', eventType: 1, eventDimension: 1, item: 'start', loc: '0 0' },
//           { key: -832, group: 103, category: 'event', text: '结束', eventType: 1, eventDimension: 8, item: 'end', name: 'end', loc: '350 0' },
// 
//           { key: 104, text: '并行多重\n子流程', isGroup: true, isSubProcess: true, isParallel: true, category: 'subprocess', taskType: 0, loc: '0 0', item: 'multi-Instance activity' },
//           { key: -841, group: 104, category: 'event', text: '启动', eventType: 1, eventDimension: 1, item: 'start', loc: '0 0' },
//           { key: -842, group: 104, category: 'event', text: '结束', eventType: 1, eventDimension: 8, item: 'end', name: 'end', loc: '350 0' },
//           { key: 105, text: '时序多重\n子流程', isGroup: true, isSubProcess: true, isSequential: true, category: 'subprocess', taskType: 0, loc: '0 0', item: 'sequential activity' },
//           { key: -851, group: 105, category: 'event', text: '启动', eventType: 1, eventDimension: 1, item: 'start', loc: '0 0' },
//           { key: -852, group: 105, category: 'event', text: '结束', eventType: 1, eventDimension: 8, item: 'end', name: 'end', loc: '350 0' },
// 
//           { key: 106, text: '调用活动', category: 'activity', isSubProcess: true, taskType: 0, isCall: true, loc: '0 0', item: 'call activity' },
...
// 
//           // gateway nodes
//           { key: 301, category: 'gateway', gatewayType: 2, text: '包含网关' },
//           { key: 302, category: 'gateway', gatewayType: 5, text: '时间网关' },
// 
//           // events
//           { key: 401, category: 'event', eventType: 5, eventDimension: 1, text: '条件启动', item: 'BpmnEventConditional' },
//           { key: 402, category: 'event', eventType: 10, eventDimension: 1, text: '信号启动', item: 'BpmnEventSignal' }, // start signal
//           { key: 403, category: 'event', eventType: 10, eventDimension: 8, text: '信号结束', item: 'end signal' },
//           { key: 404, category: 'event', eventType: 7, eventDimension: 8, text: '错误结束', item: 'BpmnEventError' },
//           { key: 405, category: 'event', eventType: 4, eventDimension: 8, text: '升级结束', item: 'BpmnEventEscalation' },
//           // throwing / catching intermedicate events
//           { key: 502, category: 'event', eventType: 6, eventDimension: 4, text: '链接中间\n捕获', item: 'BpmnEventOffPage' },
//           { key: 503, category: 'event', eventType: 6, eventDimension: 7, text: '链接\n中间抛出', item: 'BpmnEventOffPage' },
//           { key: 504, category: 'event', eventType: 2, eventDimension: 4, text: '消息\n中间捕获', item: 'Message' },
//           { key: 505, category: 'event', eventType: 2, eventDimension: 7, text: '消息\n中间抛出', item: 'Message' },
//           { key: 506, category: 'event', eventType: 5, eventDimension: 4, text: '条件\n中间捕获', item: '' },
//           { key: 507, category: 'event', eventType: 3, eventDimension: 4, text: '定时器\n中间捕获', item: '' },
//           { key: 508, category: 'event', eventType: 4, eventDimension: 7, text: '升级\n中间抛出', item: 'Escalation' },
//           { key: 509, category: 'event', eventType: 10, eventDimension: 4, text: '信号\n中间捕获', item: '' },
//           { key: 510, category: 'event', eventType: 10, eventDimension: 7, text: '信号\n中间抛出', item: '' }
//         ] // end nodeDataArray
//       }) // end model
...
// 
//           { key: 111, category: 'event', eventType: 11, eventDimension: 1, text: '多重启动', item: 'Multiple' },
//           { key: 112, category: 'event', eventType: 12, eventDimension: 1, text: '并行启动', item: 'Parallel' },
// 
//           { key: 119, category: 'event', eventType: 9, eventDimension: 5, text: '补偿\n中间事件', item: 'BpmnEventCompensation' },
// 
//           // activity nodes
//           { key: 123, category: 'activity', taskType: 3, text: '手动任务', item: 'Manual Task' },
//           { key: 124, category: 'activity', taskType: 4, isSequential: true, text: '时序\n脚本任务', item: 'Script Task' },
//           { key: 125, category: 'activity', taskType: 5, isParallel: true, text: '并行\n发送任务', item: 'Send Msg Task' },
//           { key: 126, category: 'activity', taskType: 6, isLoop: true, text: '循环\n服务任务', item: 'service task' },
// 
//           // gateway nodes not in Level 1 or Level 2
//           { key: 163, category: 'gateway', text: '复杂网关', gatewayType: 3 },
//           { key: 156, category: 'gateway', text: '排他开始\n网关', gatewayType: 6 },
//           { key: 157, category: 'gateway', text: '并行开始\n网关', gatewayType: 7 }
// 
          // {
          //   key: 4,
          //   category: 'activity',
      ...
      .then((response) => {
        ...
        if (code === 1) {
          for (let folder of response.data.data) {
            mySelf.folders.push({text: folder.folderName, value: folder.folderId})
          }
        }
      })
      .catch((error) => {
  ...
  methods: {
          ...
          switch (node.category) {
            case 'activity': {
              ...
              if (node.boundaryEventArray === undefined) {
                ...
                for (let b = 0; b < boundaryArray.length; b++) {
                  ...
                  if (belongToLane === true) {
                    ...
                    xmlDoc.getElementById(currentProcess).insertBefore(boundary, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
                  } else {
                    if (node.group !== undefined) {
              ...
              if (belongToLane === true) {
                ...
                flowNodeRef.append(node.key)
                xmlDoc.getElementById(node.group).appendChild(flowNodeRef)
                xmlDoc.getElementById(currentProcess).insertBefore(task, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
              } else {
                if (node.group !== undefined) {
            ...
            case 'event': {
              ...
              for (let j = 0; j < linkArray.length; j++) {
                if (!sequenceFlowIdMapping[j].toString().startsWith('Sequence')) {
                  if (sequenceFlowIdMapping[j].toString().startsWith('datastore') || sequenceFlowIdMapping[j].toString().startsWith('dataobject')) {
                    if (linkArray[j].from.toString() === node.key) {
                      datastoreOut.push(j)
                    }
                    if (linkArray[j].to.toString() === node.key) {
              ...
              if (belongToLane === true) {
                ...
                flowNodeRef.append(node.key)
                xmlDoc.getElementById(node.group).appendChild(flowNodeRef)
                xmlDoc.getElementById(currentProcess).insertBefore(event, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
              } else {
                if (node.group !== undefined) {
            ...
            case 'subprocess': {
              ...
              if (belongToLane === true) {
                ...
                flowNodeRef.append(node.key)
                xmlDoc.getElementById(node.group).appendChild(flowNodeRef)
                xmlDoc.getElementById(currentProcess).insertBefore(subprocess, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
              } else {
                if (node.group !== undefined) {
            ...
            case 'gateway': {
              ...
              if (belongToLane === true) {
                ...
                flowNodeRef.append(node.key)
                xmlDoc.getElementById(node.group).appendChild(flowNodeRef)
                xmlDoc.getElementById(currentProcess).insertBefore(gateway, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
              } else {
                if (node.group !== undefined) {
            ...
            case 'dataobject': {
              ...
              if (belongToLane === true) {
                xmlDoc.getElementById(currentProcess).insertBefore(dataobject, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
                xmlDoc.getElementById(currentProcess).insertBefore(dataObjectReference, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
              } else {
                if (node.group !== undefined) {
            ...
            case 'datastore': {
              ...
              if (belongToLane === true) {
                xmlDoc.getElementById(currentProcess).insertBefore(datastore, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
              } else {
                if (node.group !== undefined) {
            ...
            case 'annotation': {
              ...
              if (belongToLane === true) {
                xmlDoc.getElementById(currentProcess).insertBefore(annotation, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
              } else {
                if (node.group !== undefined) {
              ...
              for (let i = 0; i < sequenceFlowIdMapping.length; i++) {
                if (sequenceFlowIdMapping[i].toString().startsWith('annotation') && (linkArray[i].to.toString() === node.key || linkArray[i].from.toString() === node.key)) {
                  ...
                  if (belongToLane === true) {
                    xmlDoc.getElementById(currentProcess).insertBefore(association, xmlDoc.getElementById(currentProcess).getElementsByTagName('bpmn:sequenceFlow')[0])
                  } else {
                    if (node.group !== undefined) {
    ...
    BPMNxml2Json (xmlText) {
      ...
      json.modelData = {'position': diagram.getAttribute('modelData').toString()}
      let nodeArray = []
      let linkArray = []
